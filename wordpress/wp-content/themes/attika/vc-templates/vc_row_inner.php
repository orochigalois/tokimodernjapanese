...
$row_content_width = $anchor = $content_text_aligment = $simple_background_color = $simple_background_image = $background_image_position = $disable_background_image = '';
...
if ( $row_content_width !== 'grid' ) {
	...
	if ( ! empty( $simple_background_image ) ) {
	    $css_classes[]      = 'mkdf-row-inner-with-background-image';
...
$mkdf_parallax_elements = '';
$mkdf_parallax_elements_html = '';

if ($enable_parallax_effect === 'yes') {
	$grid_row_data[] = 'data-parallax={"y":-80,"smoothness":30}';
}
...
if ($enable_parallax_background_elements === 'yes') {
    $mkdf_parallax_elements .= '<div class="mkdf-row-parallax-background-elements-holder mkdf-parallax-elements-'.$parallax_background_elements_vertical_alignment.'" '. esc_attr( $grid_row_datas ) .'>';
    if (!empty($left_parallax_background_element)) {
        $mkdf_parallax_elements .= '<div class="mkdf-row-parallax-left">';
        $mkdf_parallax_elements .= '<img class="mkdf-row-parallax-element" src="'. wp_get_attachment_url($left_parallax_background_element).'" alt="'. esc_attr__('Left Parallax Background Element','attika').'"/>';
        $mkdf_parallax_elements .= '</div>';
    }
    if (!empty($right_parallax_background_element)) {
        $mkdf_parallax_elements .= '<div class="mkdf-row-parallax-right">';
        $mkdf_parallax_elements .= '<img class="mkdf-row-parallax-element" src="'. wp_get_attachment_url($right_parallax_background_element).'" alt="'. esc_attr__('Right Parallax Background Element','attika').'"/>';
        $mkdf_parallax_elements .= '</div>';
    }
    $mkdf_parallax_elements .= '</div>';


    $mkdf_parallax_elements_html .= $mkdf_parallax_elements;
}

if ( $row_content_width === 'grid' ) {
	$mkdf_row_wrapper_start .= '<div class="mkdf-row-grid-section-wrapper ' . esc_attr( $grid_row_classes ) . '" ' . attika_mikado_get_inline_style( $mkdf_grid_row_styles ) . '><div class="mkdf-row-grid-section">';
...
$output .= '<div ' . implode( ' ', $wrapper_attributes ) . ' ' . attika_mikado_get_inline_style( $mkdf_vc_row_styles ) . '>';
$output .= wpb_js_remove_wpautop( $content );
$output .= $mkdf_parallax_elements_html;
$output .= '</div>';
$output .= $mkdf_row_wrapper_end;
$output .= $after_output;

print attika_mikado_get_module_part($output);
