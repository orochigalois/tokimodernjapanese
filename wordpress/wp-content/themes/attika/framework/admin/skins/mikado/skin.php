...
class MikadoSkin extends AttikaMikadoClassSkinAbstract {
	
	/**
	 * Skin constructor. Hooks to attika_mikado_admin_scripts_init and attika_mikado_enqueue_admin_styles
	 */
	public function __construct() {
		$this->skinName = 'mikado';
		
		//hook to
		add_action( 'attika_mikado_action_admin_scripts_init', array( $this, 'registerStyles' ) );
		add_action( 'attika_mikado_action_admin_scripts_init', array( $this, 'registerScripts' ) );
		
		add_action( 'attika_mikado_action_enqueue_admin_styles', array( $this, 'enqueueStyles' ) );
		add_action( 'attika_mikado_action_enqueue_admin_scripts', array( $this, 'enqueueScripts' ) );
		
		add_action( 'attika_mikado_action_enqueue_meta_box_styles', array( $this, 'enqueueStyles' ) );
		add_action( 'attika_mikado_action_enqueue_meta_box_scripts', array( $this, 'enqueueScripts' ) );
		
		add_filter( 'attika_mikado_filter_options_map_position', array( $this, 'setOptionsMapPosition' ), 10, 2 );
	...
	public function registerScripts() {
		...
		foreach ( $this->scripts as $scriptHandle => $script ) {
			attika_mikado_register_skin_script( $scriptHandle, $script['path'], $script['dependency'] );
		}
	}
    ...
    public function registerStyles() {
        ...
        foreach ($this->styles as $styleHandle => $stylePath) {
            attika_mikado_register_skin_style($styleHandle, $stylePath);
        }
    }
    ...
    public function renderOptions() {
        ...
        $tab    		= attika_mikado_get_admin_tab();
        $active_page 	= $attika_mikado_global_Framework->mkdOptions->getAdminPageFromSlug($tab);
     ...
     *
     * @see AttikaMikadoClassSkinAbstract::loadTemplatePart()
     */
     ...
     *
     * @param AttikaMikadoClassAdminPage $page current page to load
	...
	public function renderBackupOptions() { ?>
		<div class="mkdf-options-page mkdf-page">
			<?php $this->getHeader(attika_mikado_get_theme_info_item('Name'), attika_mikado_get_theme_info_item('Version'), false); ?>
			<div class="mkdf-page-content-wrapper">
				<div class="mkdf-page-content">
					<div class="mkdf-page-navigation mkdf-tabs-wrapper vertical left clearfix">
