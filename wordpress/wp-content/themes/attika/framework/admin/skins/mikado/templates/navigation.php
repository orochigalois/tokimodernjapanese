<div class="mkdf-tabs-navigation-wrapper">
    <ul class="nav nav-tabs">
        <?php
        foreach (attika_mikado_options()->adminPages as $key => $page ) {
            $slug = "";
	    ...
	    <?php if (attika_mikado_core_plugin_installed()) { ?>
			<li <?php if($isBackupOptionsPage) { echo "class='active'"; } ?>><a href="<?php echo esc_url(get_admin_url().'admin.php?page='.MIKADO_OPTIONS_SLUG.'_backup_options'); ?>"><i class="fa fa-download mkdf-tooltip mkdf-inline-tooltip left" data-placement="top" data-toggle="tooltip" title="<?php esc_attr_e('Backup Options','attika'); ?>"></i><span><?php esc_html_e('Backup Options','attika'); ?></span></a></li>
	    <?php } ?>
    </ul>
</div>