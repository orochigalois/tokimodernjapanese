...
/*
   Class: AttikaMikadoClassMultipleImages
   A class that initializes Mikado Multiple Images
*/

class AttikaMikadoClassMultipleImages implements iAttikaMikadoInterfaceRender {
	private $name;
	private $label;
	private $description;
	...
	function __construct( $name, $label = "", $description = "" ) {
		...
		$attika_mikado_global_Framework->mkdMetaBoxes->addOption( $this->name, "" );
	}
					...
					<div class="row">
						<div class="col-lg-12">
							...
							<div class="mkdf-gallery-uploader">
								<a class="mkdf-gallery-upload-btn btn btn-sm btn-primary" href="javascript:void(0)"><?php esc_html_e( 'Upload', 'attika' ); ?></a>
								<a class="mkdf-gallery-clear-btn btn btn-sm btn-default pull-right" href="javascript:void(0)"><?php esc_html_e( 'Remove All', 'attika' ); ?></a>
							</div>
						</div>
					</div>
				</div>
...
class AttikaMikadoClassTwitterFramework implements iAttikaMikadoInterfaceRender {
	public function render( $factory ) {
		$twitterApi = AttikaTwitterApi::getInstance();
		$message    = '';
		...
		if ( ! empty( $_GET['oauth_token'] ) && ! empty( $_GET['oauth_verifier'] ) ) {
			...
			if ( $responseObj->status ) {
				$message = esc_html__( 'You have successfully connected with your Twitter account. If you have any issues fetching data from Twitter try reconnecting.', 'attika' );
			} else {
				$message = $responseObj->message;
			}
		}
		
		$buttonText = $twitterApi->hasUserConnected() ? esc_html__( 'Re-connect with Twitter', 'attika' ) : esc_html__( 'Connect with Twitter', 'attika' );
		?>
		<?php if ( $message !== '' ) { ?>
		...
		<div class="mkdf-page-form-section" id="mkdf_enable_social_share">
			<div class="mkdf-field-desc">
				<h4><?php esc_html_e( 'Connect with Twitter', 'attika' ); ?></h4>
				<p><?php esc_html_e( 'Connecting with Twitter will enable you to show your latest tweets on your site', 'attika' ); ?></p>
			</div>
			<div class="mkdf-section-content">
				<div class="container-fluid">
					<div class="row">
...
class AttikaMikadoClassInstagramFramework implements iAttikaMikadoInterfaceRender {
	public function render( $factory ) {
		$instagram_api = AttikaInstagramApi::getInstance();
		$message       = '';
		...
		if ( ! empty( $_GET['code'] ) && ! empty( $_GET['instagram'] ) ) {
			...
			$message = esc_html__( 'You have successfully connected with your Instagram Personal account.', 'attika' );
		}
		...
		if ( ! empty( $_GET['access_token'] ) && ! empty( $_GET['facebook'] ) ) {
			...
			$message = esc_html__( 'You have successfully connected with your Instagram Business account.', 'attika' );
		}
		...
		if ( ! empty( $_GET['disconnect'] ) ) {
			...
			$message = esc_html__( 'You have have been disconnected from all Instagram accounts.', 'attika' );
		...
		<div class="mkdf-page-form-section" id="mkdf_enable_social_share">
			<div class="mkdf-field-desc">
				<h4><?php esc_html_e( 'Connect with Instagram', 'attika' ); ?></h4>
				<p><?php esc_html_e( 'Connecting with Instagram will enable you to show your latest photos on your site', 'attika' ); ?></p>
			</div>
			<div class="mkdf-section-content">
				<div class="container-fluid">
					<?php
					...
					if ( $instagram_user_id ) { ?>
						<div class="row">
							<div class="col-lg-12">
								<p><?php echo esc_html__( 'You are currently connected to Instagram ID: ', 'attika' );
					...
					<div class="row">
						<?php if ( ! empty( $_GET['disconnect'] ) ) { ?>
							<div class="col-lg-4">
								<a class="btn btn-primary" href="<?php echo esc_url( $instagram_api->reloadURL() ); ?>"><?php echo esc_html__( 'Reload Page', 'attika' ); ?></a>
							</div>
						<?php } else if ( empty( $connection_type ) ) { ?>
							<div class="col-lg-4">
								<a class="btn btn-primary" href="<?php echo esc_url( $instagram_api->instagramRequestCode() ); ?>"><?php echo esc_html__( 'Connect with Instagram Personal account', 'attika' ); ?></a>
							</div>
							<div class="col-lg-4">
								<a class="btn btn-primary" href="<?php echo esc_url( $instagram_api->facebookRequestCode() ); ?>"><?php echo esc_html__( 'Connect with Instagram Business account', 'attika' ); ?></a>
							</div>
						<?php } else { ?>
							<div class="col-lg-4">
								<a class="btn btn-primary" href="<?php echo esc_url( $instagram_api->disconnectURL() ); ?>"><?php echo esc_html__( 'Disconnect Instagram account', 'attika' ) ?></a>
							</div>
						<?php } ?>
					</div>
				</div>
...
class AttikaMikadoClassRepeater implements iAttikaMikadoInterfaceRender {
	private $label;
	private $description;
	private $name;
	private $fields;
			...
			<?php if ( $this->table_layout ) { ?>
				<div class="mkdf-repeater-table-heading">
					<div class="mkdf-repeater-fields-holder">
						<div class="mkdf-repeater-table-cell mkdf-repeater-sort"><?php esc_html_e( 'Order', 'attika' ) ?></div>
						...
						<div class="mkdf-repeater-table-cell mkdf-repeater-remove"><?php esc_html_e( 'Remove', 'attika' ) ?></div>
					</div>
				</div>
			<?php } ?>
																			...
																			<div class="mkdf-repeater-inner-fields-row <?php echo esc_attr( $col_width_inner_class ); ?>">
																				<div class="mkdf-repeater-inner-fields-row-inner">
																					...
																					if ( ! empty( $field_inner['dependency'] ) ) {
																						$dependencyValues = attika_mikado_return_repeater_dependency_options_array(array(
																							'field' 	   => $field,
...
class AttikaMikadoClassFieldAddress extends AttikaMikadoClassFieldType {
					...
					<div class="row">
						<div class="col-lg-<?php echo esc_attr( $col_width ); ?>">
							...
							if ( empty( $google_maps_api_key ) ) { ?>
								<p class="description"><?php esc_html_e( 'In order for the map functionality to be enabled please input the Google Map API key in the General section of the Attika Options', 'attika' ); ?></p>
							<?php } ?>
...
class AttikaMikadoClassFieldIcon extends AttikaMikadoClassFieldType {
	public function render( $name, $label = "", $description = "", $options = array(), $args = array(), $repeat = array() ) {
		$class = '';
		...
		$icon_packs        = attika_mikado_icon_collections()->getIconCollectionsEmpty();
