...
/*
   Class: AttikaMikadoClassDashboardForm
   A class that initializes Mikado Dashboard Container
*/

class AttikaMikadoClassDashboardForm implements iAttikaMikadoInterfaceLayoutNode, iAttikaMikadoInterfaceRender {
	public $children;
	public $name;
	public $form_id;
	public $form_method;
	...
	public function render( $factory ) {
		...
		<form method="<?php echo esc_attr( $this->form_method ); ?>" id="<?php echo esc_attr( $this->form_id ); ?>" class="<?php echo esc_attr( $action_class ) ?>" data-action="<?php echo esc_attr( $this->form_action ); ?>">
			...
			if ( attika_mikado_core_plugin_installed() ) {
				echo attika_mikado_get_button_html( array(
					'html_type'    => 'button',
...
/*
   Class: AttikaMikadoClassDashboardGroup
   A class that initializes AttikaMikadoClass Group Field
*/
class AttikaMikadoClassDashboardGroup implements iAttikaMikadoInterfaceLayoutNode, iAttikaMikadoInterfaceRender {
	public $children;
	public $name;
	public $title;
	public $description;
...
/*
   Class: AttikaMikadoClassDashboardTitle
   A class that initializes Dashboard Title
*/
class AttikaMikadoClassDashboardTitle implements iAttikaMikadoInterfaceRender {
	private $name;
	private $title;
	...
	public function render( $factory ) { ?>
		<h5 class="mkdf-dashboard-section-subtitle"
		    id="mkdf_<?php echo esc_attr( $this->name ); ?>"><?php echo esc_html( $this->title ); ?></h5>
		<?php
	}
}

/*
   Class: AttikaMikadoClassDashboardField
   A class that initializes AttikaMikadoClass Front Field
*/
class AttikaMikadoClassDashboardField implements iAttikaMikadoInterfaceRender {
	private $type;
	private $name;
	private $label;
	private $description;
...
abstract class AttikaMikadoClassDashboardFieldType {
	abstract public function render( $name, $label = "", $description = "", $options = array(), $args = array(), $value = "" );
}

class AttikaMikadoClassDashboardFieldText extends AttikaMikadoClassDashboardFieldType {
...
class AttikaMikadoClassDashboardFieldTextArea extends AttikaMikadoClassDashboardFieldType {
	public function render( $name, $label = "", $description = "", $options = array(), $args = array(), $value = '', $repeat = array() ) {
		$col_width = 12;
...
class AttikaMikadoClassDashboardFieldImage extends AttikaMikadoClassDashboardFieldType {
	public function render( $name, $label = "", $description = "", $options = array(), $args = array(), $value = '', $repeat = array() ) {
		$class = '';
			...
			<div class="mkdf-dashboard-gallery-holder">
				<ul class="mkdf-dashboard-gallery-images-holder">
					<?php if ( isset( $value_html ) ) {
						print attika_mikado_get_module_part($value_html);
					} ?>
				</ul>
				<div class="mkdf-dashboard-gallery-uploader">
					<?php
					if ( attika_membership_theme_installed() ) {
						echo attika_mikado_get_button_html( array(
							'text'         => esc_html__( 'Upload', 'attika' ),
							'custom_class' => 'mkdf-dashboard-gallery-upload'
						) );
					} else {
						echo '<a itemprop="url" href="#" class="mkdf-btn mkdf-btn-medium mkdf-btn-solid mkdf-dashboard-gallery-upload"><span class="mkdf-btn-text">' . esc_html__( 'Upload', 'attika' ) . '</span></a>';
					} ?>
					...
					<?php if ( $value !== '' && $value !== false ) { ?>
						<button class="mkdf-btn mkdf-btn-solid mkdf-dashboard-remove-image"><?php esc_html_e( 'Remove Media', 'attika' ); ?></button>
					<?php } ?>
				</div>
			</div>
		</div>
...
class AttikaMikadoClassDashboardFieldGallery extends AttikaMikadoClassDashboardFieldType {
	public function render( $name, $label = "", $description = "", $options = array(), $args = array(), $value = '', $repeat = array() ) {
		$class = '';
...
class AttikaMikadoClassDashboardFieldSelect extends AttikaMikadoClassDashboardFieldType {
	public function render( $name, $label = "", $description = "", $options = array(), $args = array(), $value = '', $repeat = array() ) {
		$class = '';
...
class AttikaMikadoClassDashboardFieldDate extends AttikaMikadoClassFieldType {
	public function render( $name, $label = "", $description = "", $options = array(), $args = array(), $value = '', $repeat = array() ) {
		if ( ! empty( $repeat ) && array_key_exists( 'name', $repeat ) && array_key_exists( 'index', $repeat ) ) {
			$id   = $name . '-' . $repeat['index'];
			$name = $repeat['name'] . '[' . $repeat['index'] . '][' . $name . ']';
...
class AttikaMikadoClassDashboardFieldIcon extends AttikaMikadoClassDashboardFieldType {
	public function render( $name, $label = "", $description = "", $options = array(), $args = array(), $value = '', $repeat = array() ) {
		$options           = attika_mikado_icon_collections()->getIconCollectionsEmpty();
		$icons_collections = attika_mikado_icon_collections()->getIconCollectionsKeys();
				...
				<div class="mkdf-dashboard-item">
					...
					<div class="mkdf-dashboard-icon-holder">
						...
						<?php foreach ( $icons_collections as $icons_collection ) {
							$icons_param = attika_mikado_icon_collections()->getIconCollectionParamNameByKey( $icons_collection );
							$style       = 'display: none';
							...
							<div class="mkdf-icon-collection-holder" style="<?php echo esc_attr( $style ); ?>" data-icon-collection="<?php echo esc_attr( $icons_collection ); ?>">
								<select name="<?php echo esc_attr( $name . '[' . $icons_param . ']' ); ?>" id="<?php echo esc_attr( $name . '[' . $icons_param . ']' ); ?>">
									<?php
									$icons       = attika_mikado_icon_collections()->getIconCollection( $icons_collection );
...
class AttikaMikadoClassDashboardFieldColor extends AttikaMikadoClassDashboardFieldType {
	public function render( $name, $label = "", $description = "", $options = array(), $args = array(), $value = '', $repeat = array() ) {
		if ( ! empty( $repeat ) && array_key_exists( 'name', $repeat ) && array_key_exists( 'index', $repeat ) ) {
			$id   = $name . '-' . $repeat['index'];
			$name = $repeat['name'] . '[' . $repeat['index'] . '][' . $name . ']';
...
class AttikaMikadoClassDashboardFieldCheckBoxGroup extends AttikaMikadoClassDashboardFieldType {
	public function render( $name, $label = "", $description = "", $options = array(), $args = array(), $value = '', $repeat = array() ) {
		$col_width = 12;
...
class AttikaMikadoClassDashboardFieldAddress extends AttikaMikadoClassFieldType {
	public function render( $name, $label = "", $description = "", $options = array(), $args = array(), $value = '', $repeat = array() ) {
		$col_width = 12;
		if ( isset( $args["col_width"] ) ) {
			$col_width = $args["col_width"];
			...
			<div class="mkdf-dashboard-field-row">
				<div class="mkdf-dashboard-item col-lg-<?php echo esc_attr( $col_width ); ?>">
					<div class="mkdf-dashboard-input-holder">
						...
						<a id="reset" href="#" style="display:none;"><?php esc_html_e( 'Reset Marker', 'attika' ); ?></a>
					</div>
				</div>
			</div>
		</div>
...
class AttikaMikadoClassDashboardRepeater implements iAttikaMikadoInterfaceRender {
	private $label;
	private $description;
	private $name;
	private $fields;
	...
	function __construct( $fields, $name, $label = '', $description = '', $button_text = '', $table_layout = false, $value = array() ) {
		...
		$this->name         = $name;
		$this->num_of_rows  = 1;
		$this->button_text  = ! empty( $button_text ) ? $button_text : esc_html__( 'Add New Item', 'attika' );
			...
			<?php if ( $this->table_layout ) { ?>
				<div class="mkdf-dashboard-repeater-table-heading">
					<div class="mkdf-dashboard-repeater-fields-holder">
						<div class="mkdf-dashboard-repeater-table-cell mkdf-dashboard-repeater-sort"><?php esc_html_e( 'Order', 'attika' ) ?></div>
						...
						<div class="mkdf-dashboard-repeater-table-cell mkdf-dashboard-repeater-remove"><?php esc_html_e( 'Remove', 'attika' ) ?></div>
					</div>
				</div>
			<?php } ?>
...
class AttikaMikadoClassDashboardFieldFactory {
	public function render( $field_type, $name, $label = "", $description = "", $options = array(), $args = array(), $value = '', $repeat = array() ) {
		switch ( strtolower( $field_type ) ) {
			case 'text':
				$field = new AttikaMikadoClassDashboardFieldText();
				$field->render( $name, $label, $description, $options, $args, $value, $repeat );
				break;
			
			case 'textarea':
				$field = new AttikaMikadoClassDashboardFieldTextArea();
				$field->render( $name, $label, $description, $options, $args, $value, $repeat );
				break;
			
			case 'date':
				$field = new AttikaMikadoClassDashboardFieldDate();
				$field->render( $name, $label, $description, $options, $args, $value, $repeat );
				break;
			
			case 'image':
				$field = new AttikaMikadoClassDashboardFieldImage();
				$field->render( $name, $label, $description, $options, $args, $value, $repeat );
				break;
			
			case 'gallery':
				$field = new AttikaMikadoClassDashboardFieldGallery();
				$field->render( $name, $label, $description, $options, $args, $value, $repeat );
				break;
			
			case 'select':
				$field = new AttikaMikadoClassDashboardFieldSelect();
				$field->render( $name, $label, $description, $options, $args, $value, $repeat );
				break;
			
			case 'icon':
				$field = new AttikaMikadoClassDashboardFieldIcon();
				$field->render( $name, $label, $description, $options, $args, $value, $repeat );
				break;
			
			case 'color':
				$field = new AttikaMikadoClassDashboardFieldColor();
				$field->render( $name, $label, $description, $options, $args, $value, $repeat );
				break;
			
			case 'checkboxgroup':
				$field = new AttikaMikadoClassDashboardFieldCheckBoxGroup();
				$field->render( $name, $label, $description, $options, $args, $value, $repeat );
				break;
			
			case 'address':
				$field = new AttikaMikadoClassDashboardFieldAddress();
