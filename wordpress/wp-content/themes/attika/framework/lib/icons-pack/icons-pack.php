...
foreach ( glob( MIKADO_FRAMEWORK_ICONS_ROOT_DIR . '/*/load.php' ) as $module_load ) {
	if ( attika_mikado_is_customizer_item_enabled( $module_load, 'attika_performance_disable_icon_pack_' ) ) {
		include_once $module_load;
	}
}

if ( ! function_exists( 'attika_mikado_get_icon_pack_collections' ) ) {
	function attika_mikado_get_icon_pack_collections() {
		$options = apply_filters( 'attika_mikado_filter_add_icon_pack_into_collection', $options = array() );
		
		return $options;
	}
	
	add_action( 'after_setup_theme', 'attika_mikado_get_icon_pack_collections' );
}

if ( ! function_exists( 'attika_mikado_get_icon_pack_options' ) ) {
	function attika_mikado_get_icon_pack_options() {
		$options = apply_filters( 'attika_mikado_filter_add_icon_pack_into_options', $options = array() );
		
		return $options;
	}
	
	add_action( 'after_setup_theme', 'attika_mikado_get_icon_pack_options' );
}

if ( ! function_exists( 'attika_mikado_get_icon_pack_social_options' ) ) {
	function attika_mikado_get_icon_pack_social_options() {
		$options = apply_filters( 'attika_mikado_filter_add_icon_pack_into_social_options', $options = array() );
		
		return $options;
	}
	
	add_action( 'after_setup_theme', 'attika_mikado_get_icon_pack_social_options' );
}

/*
 * Class: AttikaMikadoClassIconCollections
 */
class AttikaMikadoClassIconCollections {
	...
	private function __construct() {
		...
		$this->iconPackCollections   = attika_mikado_get_icon_pack_collections();
		$this->iconPackOptions       = attika_mikado_get_icon_pack_options();
		$this->iconPackSocialOptions = attika_mikado_get_icon_pack_social_options();
	...
	public function getIconsMetaBoxOrOption( $attributes ) {
		...
		foreach ( $icon_collections as $collection_key => $collection_object ) {
			...
			$icon_pack_container = attika_mikado_add_admin_container(
				array(
					'parent'     => $parent,
	...
	public function getVCParamsArray( $iconPackDependency = array(), $iconCollectionPrefix = "", $emptyIconPack = false ) {
		...
		$iconPackParams = array(
			'type'        => 'dropdown',
			'heading'     => esc_html__( 'Icon Pack', 'attika' ),
		...
		if ( is_array( $this->iconCollections ) && count( $this->iconCollections ) ) {
			foreach ( $this->iconCollections as $key => $collection ) {
				$iconSetParams[] = array(
					'type'        => 'dropdown',
					'heading'     => esc_html__( 'Icon', 'attika' ),
	...
	public function getIconWidgetParamsArray() {
		$iconPackParams[] = array(
			'type'    => 'dropdown',
			'name'    => 'icon_pack',
			'title'   => esc_html__( 'Icon Pack', 'attika' ),
			'options' => $this->getIconPackOptions()
		);
		...
		if ( is_array( $this->iconCollections ) && count( $this->iconCollections ) ) {
			foreach ( $this->iconCollections as $key => $collection ) {
				$iconSetParams[] = array(
					'type'    => 'dropdown',
					'title'   => $collection->title . esc_html__( ' Icon', 'attika' ),
	...
	public function getSocialIconWidgetMultipleParamsArray( $count ) {
		...
		for ( $n = 1; $n <= $count; $n ++ ) {
			if ( is_array( $iconCollectionsVC ) && count( $iconCollectionsVC ) ) {
				foreach ( $iconCollectionsVC as $key => $collection ) {
					$iconOps[] = array(
						...
						'title'   => sprintf( esc_html__( 'Icon %s %s Icon', 'attika' ), $n, $collection->title ),
			...
			$iconOps[] = array(
				'type'  => 'textfield',
				'name'  => 'link_' . $n,
				'title' => sprintf( esc_html__( 'Link %s', 'attika' ), $n )
			);
			
			$iconOps[] = array(
				'type'    => 'dropdown',
				'name'    => 'target_' . $n,
				'title'   => sprintf( esc_html__( 'Link Target %s', 'attika' ), $n ),
	...
	public function getIconSizesArray() {
		return array(
			esc_html__( 'Tiny', 'attika' )       => 'fa-lg',
			esc_html__( 'Small', 'attika' )      => 'fa-2x',
			esc_html__( 'Medium', 'attika' )     => 'fa-3x',
			esc_html__( 'Large', 'attika' )      => 'fa-4x',
			esc_html__( 'Very Large', 'attika' ) => 'fa-5x'
		);
	}
	...
	public function getIconCollectionIcons( iAttikaMikadoInterfaceIconCollection $collection ) {
	...
	public function getIconCollectionsEmpty( $no_empty_key = "" ) {
		$array                  = array();
		$array[ $no_empty_key ] = esc_html__( 'No Icon', 'attika' );
	...
	public function getIconCollectionsVCEmpty() {
		$vc_array                                           = array();
		$vc_array[ esc_html__( 'No Icon', 'attika' ) ] = '';
	...
	public function enqueueStyles() {
		if ( is_array( $this->iconCollections ) && count( $this->iconCollections ) ) {
			foreach ( $this->iconCollections as $collection_key => $collection_obj ) {
				wp_enqueue_style( 'attika-mikado-' . $collection_key, $collection_obj->styleUrl );
			}
		}
	}
