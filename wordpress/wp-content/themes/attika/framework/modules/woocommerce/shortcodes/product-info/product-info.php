<?php
namespace AttikaCore\CPT\Shortcodes\ProductInfo;
...
class ProductInfo implements Lib\ShortcodeInterface {
	...
	public function vcMap() {
		if(function_exists('vc_map')) {
			vc_map(
				array(
					'name'                      => esc_html__( 'Product Info', 'attika' ),
					'base'                      => $this->getBase(),
					...
					'params'                    => array(
						array(
							...
							'heading'     => esc_html__( 'Selected Product', 'attika' ),
							'settings'    => array(
								'sortable'      => true,
								'unique_values' => true
							),
							'admin_label' => true,
							'save_always' => true,
							'description' => esc_html__( 'If you left this field empty then product ID will be of the current page', 'attika' )
						),
						array(
							'type'        => 'dropdown',
							'param_name'  => 'product_info_type',
							'heading'     => esc_html__( 'Product Info Type', 'attika' ),
							'value'       => array(
								esc_html__( 'Title', 'attika' )          => 'title',
								esc_html__( 'Featured Image', 'attika' ) => 'featured_image',
								esc_html__( 'Category', 'attika' )       => 'category',
								esc_html__( 'Excerpt', 'attika' )        => 'excerpt',
								esc_html__( 'Price', 'attika' )          => 'price',
								esc_html__( 'Rating', 'attika' )         => 'rating',
								esc_html__( 'Add To Cart', 'attika' )    => 'add_to_cart',
								esc_html__( 'Tag', 'attika' )            => 'tag',
								esc_html__( 'Author', 'attika' )         => 'author',
								esc_html__( 'Date', 'attika' )           => 'date',
							),
							'admin_label' => true
						),
						array(
							...
							'heading'    => esc_html__( 'Product Info Color', 'attika' ),
						...
						array(
							...
							'description' => esc_html__( 'Set title tag for product title element', 'attika' ),
						...
						array(
							...
							'heading'    => esc_html__( 'Featured Image Proportions', 'attika' ),
							'value'      => array(
								esc_html__( 'Default', 'attika' )        => '',
								esc_html__( 'Original', 'attika' )       => 'full',
								esc_html__( 'Square', 'attika' )         => 'attika_mikado_image_square',
								esc_html__( 'Landscape', 'attika' )      => 'attika_mikado_image_landscape',
								esc_html__( 'Portrait', 'attika' )       => 'attika_mikado_image_portrait',
						...
						array(
							...
							'heading'     => esc_html__( 'Category Tag', 'attika' ),
							'value'       => array_flip( attika_mikado_get_title_tag( true ) ),
							'description' => esc_html__( 'Set category tag for product category element', 'attika' ),
						...
						array(
							...
							'heading'    => esc_html__( 'Add To Cart Skin', 'attika' ),
							'value'      => array(
								esc_html__( 'Default', 'attika' ) => '',
								esc_html__( 'White', 'attika' )   => 'white',
						...
						array(
							...
							'heading'    => esc_html__( 'Add To Cart Size', 'attika' ),
							'value'      => array(
								esc_html__( 'Default', 'attika' ) => '',
							...
							'dependency' => array( 'element' => 'product_info_type', 'value'   => array( 'attika' ) ),
							'group'      => esc_html__( 'Product Info Style', 'attika' )
						)
					)
				)
	...
	public function getItemTitleHtml( $params ) {
		...
		if ( ! empty( $title ) ) {
			$html = '<' . esc_attr( $title_tag ) . ' itemprop="name" class="mkdf-pi-title entry-title" ' . attika_mikado_get_inline_style( $product_info_style ) . '>';
			...
			$html .= '<div class="mkdf-pi-category">';
				foreach ( $categories as $cat ) {
					if ( ! empty( $params['category_tag'] ) ) {
						$html .= '<' . esc_attr( $params['category_tag'] ) . ' ' . attika_mikado_get_inline_style( $product_info_style ) . '>';
						...
						$html .= '<a itemprop="url" class="mkdf-pi-category-item" href="' . esc_url( get_term_link( $cat->term_id ) ) . '" ' . attika_mikado_get_inline_style( $product_info_style ) . '>' . esc_html( $cat->name ) . '</a>';
					}
				}
			$html .= '</div>';
		}
	...
	public function getItemExcerptHtml( $params ) {
		...
		if ( ! empty( $excerpt ) ) {
			$html = '<div class="mkdf-pi-excerpt"><p itemprop="description" class="mkdf-pi-excerpt-item" ' . attika_mikado_get_inline_style( $product_info_style ) . '>' . esc_html( $excerpt ) . '</p></div>';
		}
	...
	public function getItemPriceHtml( $params ) {
		...
		if ( $price_html = $product->get_price_html() ) {
			$html = '<div class="mkdf-pi-price" ' . attika_mikado_get_inline_style( $product_info_style ) . '>' . $price_html . '</div>';
		}
	...
	public function getItemRatingHtml( $params ) {
		...
		if ( get_option( 'woocommerce_enable_review_rating' ) !== 'no' ) {
			...
			$html = '<div class="mkdf-pi-rating" title="' . sprintf( esc_attr__( "Rated %s out of 5", "attika" ), $average ) . '" ' . attika_mikado_get_inline_style( $product_info_style ) . '><span style="width:' . ( ( $average / 5 ) * 100 ) . '%"></span></div>';
		}
	...
	public function getItemAddToCartHtml( $params ) {
		...
		$html .= apply_filters( 'attika_mikado_filter_product_info_add_to_cart_link',
	...
	public function getItemTagHtml( $params ) {
		...
		if ( ! empty( $tags ) ) {
			$html = '<div class="mkdf-pi-tag">';
				foreach ( $tags as $tag ) {
					$html .= '<a itemprop="url" class="mkdf-pi-tag-item" href="' . esc_url( get_term_link( $tag->term_id ) ) . '" ' . attika_mikado_get_inline_style( $product_info_style ) . '>' . esc_html( $tag->name ) . '</a>';
				}
			$html .= '</div>';
		}
	...
	public function getItemAuthorHtml( $params ) {
		...
		if ( ! empty( $author ) ) {
			$html = '<div class="mkdf-pi-author" ' . attika_mikado_get_inline_style( $product_info_style ) . '>' . esc_html( $author ) . '</div>';
		}
	...
	public function getItemDateHtml( $params ) {
		...
		if ( ! empty( $date ) ) {
			$html = '<div class="mkdf-pi-date" ' . attika_mikado_get_inline_style( $product_info_style ) . '>' . esc_html( $date ) . '</div>';
		}
	...
	public function productIdAutocompleteSuggester( $query ) {
		...
		if ( is_array( $post_meta_infos ) && ! empty( $post_meta_infos ) ) {
			foreach ( $post_meta_infos as $value ) {
				$data = array();
				$data['value'] = $value['id'];
				$data['label'] = esc_html__( 'Id', 'attika' ) . ': ' . $value['id'] . ( ( strlen( $value['title'] ) > 0 ) ? ' - ' . esc_html__( 'Title', 'attika' ) . ': ' . $value['title'] : '' );
				$results[] = $data;
			}
		}
		...
		if ( ! empty( $query ) ) {
			...
			if ( ! is_wp_error( $product ) ) {
				...
				if ( ! empty( $product_title ) ) {
					$product_title_display = ' - ' . esc_html__( 'Title', 'attika' ) . ': ' . $product_title;
				}
				
				$product_id_display = esc_html__( 'Id', 'attika' ) . ': ' . $product_id;
