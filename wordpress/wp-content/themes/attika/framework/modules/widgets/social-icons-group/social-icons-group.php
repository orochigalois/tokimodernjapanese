...
if ( class_exists( 'AttikaCoreClassWidget' ) ) {
    class AttikaMikadoClassClassIconsGroupWidget extends AttikaCoreClassWidget {
        public function __construct() {
            parent::__construct(
                'mkdf_social_icons_group_widget',
                esc_html__( 'Attika Social Icons Group Widget', 'attika' ),
                array( 'description' => esc_html__( 'Use this widget to add a group of up to 6 social icons to a widget area.', 'attika' ) )
            );
        ...
        protected function setParams() {
            $this->params = array_merge(
                ...
                attika_mikado_icon_collections()->getSocialIconWidgetMultipleParamsArray(6),
                array(
                    array(
                        'type'    => 'dropdown',
                        'name'    => 'layout',
                        'title'   => esc_html__( 'Icons Layout', 'attika' ),
                        'options' => array(
                            ''             => esc_html__( 'Default', 'attika' ),
                            'square-icons' => esc_html__( 'Square', 'attika' ),
                        )
                    ),
                    array(
                        'type'        => 'dropdown',
                        'name'        => 'skin',
                        'title'       => esc_html__( 'Square Icons Skin', 'attika' ),
                        'description' => esc_html__( 'This applies to the square layout', 'attika' ),
                        'options'     => array(
                            ''           => esc_html__( 'Dark Skin', 'attika' ),
                            'light-skin' => esc_html__( 'Light Skin', 'attika' ),
                        )
                    ),
                    array(
                        'type'    => 'dropdown',
                        'name'    => 'alignment',
                        'title'   => esc_html__( 'Text Alignment', 'attika' ),
                        'options' => array(
                    ...
                    array(
                        'type'  => 'textfield',
                        'name'  => 'icon_size',
                        'title' => esc_html__( 'Icons Size (px)', 'attika' )
                    ),
                    array(
                        'type'  => 'colorpicker',
                        'name'  => 'color',
        ...
        public function widget( $args, $instance ) {
            ...
            for ( $n = 1; $n <= 6; $n++ ) {
                ...
                <?php if ( ! empty( $icon_holder_html ) ) { ?>
                    <a class="mkdf-social-icon-widget-holder mkdf-icon-has-hover" <?php echo attika_mikado_get_inline_attr( $hover_color, 'data-hover-color' ); ?> <?php attika_mikado_inline_style( $icon_styles ) ?>
