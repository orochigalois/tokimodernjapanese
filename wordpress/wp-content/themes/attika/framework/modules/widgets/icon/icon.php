...
if ( class_exists( 'AttikaCoreClassWidget' ) ) {
    class AttikaMikadoClassIconWidget extends AttikaCoreClassWidget {
        public function __construct() {
            parent::__construct(
                'mkdf_icon_widget',
                esc_html__( 'Attika Icon Widget', 'attika' ),
                array( 'description' => esc_html__( 'Add icons to widget areas', 'attika' ) )
            );
        ...
        protected function setParams() {
            $this->params = array_merge(
                attika_mikado_icon_collections()->getIconWidgetParamsArray(),
                array(
                    array(
                        'type'  => 'textfield',
                        'name'  => 'icon_text',
                        'title' => esc_html__( 'Icon Text', 'attika' )
                    ),
                    array(
                        'type'  => 'textfield',
                        'name'  => 'link',
                    ...
                    array(
                        'type'    => 'dropdown',
                        'name'    => 'target',
                        'title'   => esc_html__( 'Target', 'attika' ),
                        'options' => attika_mikado_get_link_target_array()
                    ),
                    array(
                        'type'  => 'textfield',
                        'name'  => 'icon_size',
                        'title' => esc_html__( 'Icon Size (px)', 'attika' )
                    ),
                    array(
                        'type'  => 'colorpicker',
                        'name'  => 'icon_color',
                        'title' => esc_html__( 'Icon Color', 'attika' )
                    ),
                    array(
                        'type'  => 'colorpicker',
                        'name'  => 'icon_hover_color',
                        'title' => esc_html__( 'Icon Hover Color', 'attika' )
                    ),
                    array(
                        'type'  => 'textfield',
                        'name'  => 'text_size',
                        'title' => esc_html__( 'Text Size (px)', 'attika' )
                    ),
                    array(
                        'type'  => 'colorpicker',
                        'name'  => 'text_color',
                        'title' => esc_html__( 'Text Color', 'attika' )
                    ),
                    array(
                        'type'        => 'textfield',
                        'name'        => 'margin',
        ...
        public function widget( $args, $instance ) {
            ...
            $data   = array();
            $data[] = ! empty( $instance['icon_hover_color'] ) ? attika_mikado_get_inline_attr( $instance['icon_hover_color'], 'data-hover-color' ) : '';
            $data   = implode( ' ', $data );
            ...
            if ( ! empty( $instance['icon_size'] ) ) {
                $icon_styles[] = 'font-size: ' . attika_mikado_filter_px( $instance['icon_size'] ) . 'px';
            }

            $text_styles = array();
            if ( ! empty( $instance['text_size'] ) ) {
                $text_styles[] = 'font-size: ' . attika_mikado_filter_px( $instance['text_size'] ) . 'px';
            }
            ...
            if ( ! empty( $instance['icon_pack'] ) ) {
                ...
                if ( ! empty( $icon_class ) ) {
                    ...
                    $icon_holder_html = '<span class="mkdf-icon-element ' . esc_attr( $icon_class ) . '" ' . attika_mikado_get_inline_style( $icon_styles ) . '></span>';
                }
            }
            ...
            if ( ! empty( $instance['icon_text'] ) ) {
                $icon_text_html = '<span class="mkdf-icon-text ' . esc_attr( $icon_text_class ) . '" ' . attika_mikado_get_inline_style( $text_styles ) . '>' . esc_html( $instance['icon_text'] ) . '</span>';
            }
            ?>

            <a <?php attika_mikado_class_attribute( $holder_classes ); ?> <?php echo wp_kses_post( $data ); ?> href="<?php echo esc_url( $link ); ?>" target="<?php echo esc_attr( $target ); ?>" <?php echo attika_mikado_get_inline_style( $holder_styles ); ?>>
