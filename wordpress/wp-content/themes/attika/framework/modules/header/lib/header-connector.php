...
namespace AttikaMikadoNamespace\Modules\Header\Lib;

/**
 * Class AttikaMikadoClassHeaderConnector
 *
...
class AttikaMikadoClassHeaderConnector {
	...
	public function connect( $config = array() ) {
		do_action( 'attika_mikado_action_pre_header_connect' );
		...
		if ( ! empty( $config['affect_content'] ) ) {
			add_filter( 'attika_mikado_filter_content_elem_style_attr', array( $this, 'contentMarginFilter' ) );
		}
		
		if ( ! empty( $config['affect_title'] ) ) {
			add_filter( 'attika_mikado_filter_title_content_padding', array( $this, 'titlePaddingFilter' ) );
		}
		
		do_action( 'attika_mikado_action_after_header_connect' );
	}
	 ...
	 *
	 * Hooks to attika_mikado_filter_title_content_padding filter
	 *
	 * @return int
	 */
