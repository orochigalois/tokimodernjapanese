...
class AttikaMikadoClassLike {
	private static $instance;
	
	private function __construct() {
		add_action( 'wp_ajax_attika_mikado_like', array( $this, 'ajax' ) );
		add_action( 'wp_ajax_nopriv_attika_mikado_like', array( $this, 'ajax' ) );
	}
	...
	public function add_like() {
		...
		$title       = esc_html__( 'Like this', 'attika' );
		
		if ( isset( $_COOKIE[ 'mkdf-like_' . $post->ID ] ) ) {
			$class = 'mkdf-like liked';
			$title = esc_html__( 'You already like this!', 'attika' );
		}
...
if ( ! function_exists( 'attika_mikado_create_like' ) ) {
	function attika_mikado_create_like() {
		AttikaMikadoClassLike::get_instance();
	}
	
	add_action( 'after_setup_theme', 'attika_mikado_create_like' );
}