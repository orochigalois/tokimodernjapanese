...
if ( ! function_exists( 'attika_mikado_add_custom_nav_fields' ) ) {
	function attika_mikado_add_custom_nav_fields( $menu_item ) {
	...
	add_filter( 'wp_setup_nav_menu_item', 'attika_mikado_add_custom_nav_fields' );
}
 ...
 * @return      void
 */
if ( ! function_exists( 'attika_mikado_update_custom_nav_fields' ) ) {
	function attika_mikado_update_custom_nav_fields( $menu_id, $menu_item_db_id, $args ) {
		$check = array(
			'anchor',
			'nolink',
			'hide',
	...
	// save menu custom fields
	add_action( 'wp_update_nav_menu_item', 'attika_mikado_update_custom_nav_fields', 10, 3 );
}
 ...
 * @return      void
 */
if ( ! function_exists( 'attika_mikado_edit_walker' ) ) {
	function attika_mikado_edit_walker( $walker, $menu_id ) {
		return 'AttikaMikadoClass_Walker_Nav_Menu_Edit_Custom';
	}
	
	// edit menu walker
	add_filter( 'wp_edit_nav_menu_walker', 'attika_mikado_edit_walker', 10, 2 );
}

if ( ! function_exists( 'attika_mikado_include_custom_walkers' ) ) {
	function attika_mikado_include_custom_walkers() {
		...
		do_action( 'attika_mikado_action_include_custom_walkers_nav' );
	}
	
	add_action( 'after_setup_theme', 'attika_mikado_include_custom_walkers' );
}