...
namespace AttikaCore\CPT\Shortcodes\MasonryGallery;
...
class MasonryGallery implements Lib\ShortcodeInterface {
	...
	public function vcMap() {
		if ( function_exists( 'vc_map' ) ) {
			vc_map(
				array(
					'name'                      => esc_html__( 'Masonry Gallery', 'attika-core' ),
					'base'                      => $this->base,
					...
					'params'                    => array(
						array(
							'type'       => 'textfield',
							'param_name' => 'number',
							'heading'    => esc_html__( 'Number of Items', 'attika-core' )
						),
						array(
							'type'        => 'dropdown',
			...
			$html .= '<div class="mkdf-mg-inner mkdf-outer-space">';
				...
				if ( $query->have_posts() ) :
					while ( $query->have_posts() ) : $query->the_post();
						...
						$html .= attika_core_get_cpt_shortcode_module_template_part( 'masonry-gallery', 'masonry-gallery-list', 'masonry-gallery-' . $type . '-template', '', $params );
					
					endwhile;
				else:
					$html .= esc_html__( 'Sorry, no posts matched your criteria.', 'attika-core' );
				endif;
				wp_reset_postdata();
			$html .= '</div>';
		$html .= '</div>';
	...
	public function getItemImage( $params ) {
		...
		if ( ! empty( $imageOption ) ) {
			...
			$image_id  = attika_mikado_get_attachment_id_from_url( $image_url );
	...
	public function masonryGalleryCategoryAutocompleteSuggester( $query ) {
		...
		if ( is_array( $post_meta_infos ) && ! empty( $post_meta_infos ) ) {
			foreach ( $post_meta_infos as $value ) {
				...
				$data['label'] = ( ( strlen( $value['masonry_gallery_category_title'] ) > 0 ) ? esc_html__( 'Category', 'attika-core' ) . ': ' . $value['masonry_gallery_category_title'] : '' );
				$results[]     = $data;
			}
		}
		...
		if ( ! empty( $query ) ) {
			...
			if ( is_object( $masonry_gallery_category ) ) {
				...
				if ( ! empty( $masonry_gallery_category_title ) ) {
					$masonry_gallery_category_title_display = esc_html__( 'Category', 'attika-core' ) . ': ' . $masonry_gallery_category_title;
				}
