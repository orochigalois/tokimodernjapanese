...
namespace AttikaCore\CPT\Shortcodes\Testimonials;
...
class Testimonials implements Lib\ShortcodeInterface {
				...
				array(
					...
					'params'                    => array(
						...
						array(
							'type'       => 'textfield',
							'param_name' => 'number',
							'heading'    => esc_html__( 'Number of Testimonials', 'attika-core' )
						),
						array(
							'type'        => 'autocomplete',
							'param_name'  => 'category',
							'heading'     => esc_html__( 'Category', 'attika-core' ),
	...
	public function render( $atts, $content = null ) {
        ...
        return attika_core_get_cpt_shortcode_module_template_part( 'testimonials', 'testimonials', 'testimonials-' . $params['type'], '', $params );
	...
	private function getHolderClasses( $params ) {
		...
		$holderClasses[] = $params['enable_navigation_numbers'] === 'yes' ? 'mkdf-navigation-numbers' : '';
		$holderClasses[] = ! empty( $params['navigation_number_position'] ) ? 'mkdf-nav-' . $params['navigation_number_position'] : '';
	...
	public function testimonialsCategoryAutocompleteSuggester( $query ) {
		...
		if ( is_array( $post_meta_infos ) && ! empty( $post_meta_infos ) ) {
			foreach ( $post_meta_infos as $value ) {
				...
				$data['label'] = ( ( strlen( $value['testimonials_category_title'] ) > 0 ) ? esc_html__( 'Category', 'attika-core' ) . ': ' . $value['testimonials_category_title'] : '' );
				$results[]     = $data;
			}
		}
		...
		if ( ! empty( $query ) ) {
			...
			if ( is_object( $testimonials_category ) ) {
				...
				if ( ! empty( $testimonials_category_title ) ) {
					$testimonials_category_title_display = esc_html__( 'Category', 'attika-core' ) . ': ' . $testimonials_category_title;
				}
